...
- `stylesheet.css` — Shell UI styling.
- `schemas/org.gnome.shell.extensions.langux.gschema.xml` — GSettings schema.
- Planned (later issues): `ui/` popup (St/Clutter), `prefs.js` (GTK4/libadwaita), service layer (Soup 3).
...
cp -r metadata.json extension.js stylesheet.css ~/.local/share/gnome-shell/extensions/$UUID/
...
Run the schema compile + defaults checks and the syntax check above after every change, then
exercise the full lifecycle. The live session ignores changes until the next shell start, so
run a headless shell on an isolated bus:
